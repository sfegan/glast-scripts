...
# 2020-07-08: Update for use at LLR with new Fermi tools
...
if test "$DATAROOT" == ""
then
  DATAROOT=/data/CTA/heasarc.gsfc.nasa.gov/FTP/fermi/data/lat/weekly
fi
...
then
  DIFFUSEGALACTICMODEL=$FERMI_DIR/refdata/fermi/galdiffuse/gll_iem_v07.fits
else
  MMGALNAME="iso_user"
fi
...
then
  ISOTROPICMODEL=$FERMI_DIR/refdata/fermi/galdiffuse/iso_${IRF}${EVTYPE_ISO_FILE}_v06.txt
else
  MMDISONAME="iso_user"
fi
...
if test "$FT2" == "";
then
  FT2=$DATAROOT/spacecraft/FT2.fits
fi
