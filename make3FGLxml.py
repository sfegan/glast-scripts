...
class srcList:
	...
	def makeModel(self,GDfile="$(LATEXTDIR)/gll_iem_v05_rev1.fits",GDname='gll_iem_v05_rev1',ISOfile="$(LATEXTDIR)/iso_source_v05.txt",ISOname='iso_source_v05',normsOnly=False,extDir='',radLim=-1,maxRad=None,ExtraRad=10,sigFree=5,varFree=True,psForce=False,makeRegion=True,GIndexFree=False):
		...
		if self.srcs.split('.')[-1]=='xml':
			addSrcsXML(self,GDfile,GDname,ISOfile,ISOname)
		else:
			addSrcsFITS(self,GDfile,GDname,ISOfile,ISOname)
...
print "This is make3FGLxml version 01."
...
def addSrcsXML(sL,GD,GDn,ISO,ISOn):
		...
		if dist<=sL.roi[2]+sL.ER:
			...
			if dist>=sL.roi[2] or dist>=sL.maxRad or fixAll:
				...
				if sL.var and varIdx>=72.44:
					...
					for p in specPars:
						specOut.appendChild(parameter_element("0","%s"%str(p.getAttribute('name')),"%s"%str(p.getAttribute('max')),"%s"%str(p.getAttribute('min')),"%s"%str(p.getAttribute('scale')),"%s"%str(p.getAttribute('value'))))
			elif float(src.getAttribute('TS_value'))>=sL.sig:#hope to switch this out later for ave significance
				Sources[sname]['free']=True
				for p in specPars:
...
def addSrcsFITS(sL,GD,GDn,ISO,ISOn):
...
def cli():
	...
	parser.add_argument("-G","--galfile",type=str,default='$(LATEXTDIR)/gll_iem_v05_rev1.fits',help="Name and location of Galactic diffuse model to use, will default to 3FGL model.")
	parser.add_argument("-g","--galname",type=str,default='gll_iem_v05_rev1',help="Name of Galactic diffuse component in output model, will default to gll_iem_v06.")
	parser.add_argument("-I","--isofile",type=str,default='$(LATEXTDIR)/iso_source_v05.txt',help="Name of isotropic diffuse template for output model, will default to 3FGL model.")
	parser.add_argument("-i","--isoname",type=str,default='iso_source_v05',help="Name of isotropic diffuse component in output model, will default to iso_source_v05.")
	...
	sL.makeModel(args.galfile,args.galname,args.isofile,args.isoname,args.normsonly,args.extDir,args.radLim,args.maxRad,args.ExtraRad,args.sigFree,args.varFree,args.psForce,args.makeRegion,args.GIndexFree)
